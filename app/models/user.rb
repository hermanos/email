...
class User < ActiveRecord::Base
  ...
  def get_all_mail
    if mail_defaults
      ['INBOX', '[Gmail]/Sent Mail', '[Gmail]/Trash'].each do |folder|
  ...
  def retrieve_mail(folder)
    ...
    receive.each do |mail|
      if Email.find_by_msg_id(mail.message_id).nil?
        email = Email.create!(user_id: id, folder: folder.gsub("[Gmail]/", '').gsub(" Mail", '').downcase, msg_id: mail.message_id, from: join_address(mail.from), to: join_address(mail.to),
                              cc: join_address(mail.cc), bcc: nil, subject: mail.subject,
                              content: get_body(mail),
